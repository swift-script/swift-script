...
extension JavaScriptTranslator {
    func visit(_ n: ForInStatement) throws -> String {
        return "\(indent(of: indentLevel))for (\(n.item) of \(try n.collection.accept(JavaScriptTranslator(indentLevel: indentLevel)))) \(try translateBlock(statements: n.statements, indentLevel: indentLevel))\n"
    }
    
    func visit(_ n: WhileStatement) throws -> String {
        return "\(indent(of: indentLevel))while (\(try n.condition.accept(JavaScriptTranslator(indentLevel: indentLevel)))) \(try translateBlock(statements: n.statements, indentLevel: indentLevel))\n"
    }
    
    func visit(_ n: RepeatWhileStatement) throws -> String {
        return "\(indent(of: indentLevel))repeat \(try translateBlock(statements: n.statements, indentLevel: indentLevel)) while (\(try n.condition.accept(JavaScriptTranslator(indentLevel: indentLevel))))\n"
    }
    ...
    private func _visit(_ n: IfStatement) throws -> String {
        ...
        case let .boolean(expression):
            jsIf = "if (\(try expression.accept(JavaScriptTranslator(indentLevel: indentLevel)))) \(try translateBlock(statements: n.statements, indentLevel: indentLevel))"
            ...
            case let .else_(statements):
                return "\(jsIf) else \(try translateBlock(statements: statements, indentLevel: indentLevel))\n"
            }
    ...
    func visit(_ n: DoStatement) throws -> String {
        // TODO: catch
        return "\(indent(of: indentLevel))\(try translateBlock(statements: n.statements, indentLevel: indentLevel))\n"
    }
